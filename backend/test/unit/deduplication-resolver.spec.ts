...
describe('DeduplicationResolver (Domain Unit Tests)', () => {
  ...
  it('2. should mark business-identical observations as DUPLICATE while ignoring transport metadata', () => {
    const original = new NormalizedRecord(
      'rec-1', 'org-1', 'src-db', 'run-1', 'obs-1', 'PROD-EVT-008', 1, 'BATCH-002', 'WO-1001',
      StationCode.FOLDING, 80, new Date('2026-09-01T09:00:00Z'), QualityStatus.PASS, Disposition.ACCEPTED, null, null,
      { row_id: 8, recorded_at: '2026-09-01T09:01:00Z', folderLane: 2, sourceTable: 'production_events' },
    );

    // Same stable identity and business payload; only fixture transport fields differ.
    const duplicate = new NormalizedRecord(
      'rec-2', 'org-1', 'src-db', 'run-2', 'obs-2', 'PROD-EVT-008', 1, 'BATCH-002', 'WO-1001',
      StationCode.FOLDING, 80, new Date('2026-09-01T09:00:00Z'), QualityStatus.PASS, Disposition.ACCEPTED, null, null,
      { sourceTable: 'production_events', folderLane: 2, recorded_at: '2026-09-01T09:02:00Z', row_id: 9 },
    );
